/**
 * PERLE D'OR - Service API sécurisé
...
const API_BASE_URL =
  ...
  'https://plagiocephalic-reforgeable-sydney.ngrok-free.dev/api';
...
async function apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    const message =
      (data && (data.error || data.message)) || response.statusText || 'Une erreur est survenue';
    throw new Error(message);
  }

  // Retourne un objet vide si rien
  return data ?? {};
}
...
export const productsApi = {
  async getAll(filters: ProductFilters = {}): Promise<PaginatedResponse<Product>> {
    ...
    // Sécuriser map pour éviter crash
    const productsArray = Array.isArray(response?.data) ? response.data : [];
    const pagination = response?.pagination || { page: 1, limit: productsArray.length, total: 0, pages: 0 };

    return {
      data: productsArray.map(mapApiProduct),
      pagination,
    };
  },
...
export const categoriesApi = {
  async getAll(): Promise<{ data: Category[]; counts: Record<string, number> }> {
    ...
    const catsArray = Array.isArray(response?.data) ? response.data : [];
    ...
    return {
      data: catsArray.map(mapApiCategory),
      counts,
    };
  },
